...
setwd("/project/shared/xiao_wang/projects/MOCCA/data/Pseudotime")
opt$Exp = "/project/shared/xiao_wang/projects/MOCCA/data/Pseudotime/CPM_BC.txt"
opt$Cspot = "/project/shared/xiao_wang/projects/MOCCA/data/Pseudotime/metadata_with_cluster.csv"
opt$ImageFeature = "/project/shared/xiao_wang/projects/MOCCA/data/Pseudotime/spot_level_features.csv"
opt$numberPC = 3
opt$umap = TRUE
opt$Rratio = 0.08
opt$U = 250
...
opt$L_E = 0.625
opt$margin = 0.001
opt$diagNeighbor = TRUE
opt$scriptPath = "/project/shared/xiao_wang/projects/MOCCA/code/Spatial_denoise"
opt$outputPath = "/project/shared/xiao_wang/projects/MOCCA/data/Pseudotime/outputNew"
opt$projectID = "BC_trial"
opt$Cspot
###########  define input  ##############
cat("Inputting...\n")
if (!is.na(opt$input)){
...
counts_fn = "/project/shared/xiao_wang/projects/MOCCA/data/Pseudotime/CPM_BC.txt"
spot_meta_fn = "/project/shared/xiao_wang/projects/MOCCA/data/Pseudotime/Spot_metadata.csv"
image_features_fn = "/project/shared/xiao_wang/projects/MOCCA/data/Pseudotime/spot_level_features_intenf0f2.csv"

#########mix image feature########
IF_intens= as.matrix(read.csv("/project/shared/xiao_wang/projects/MOCCA/data/Sprod_ready_data/visium/breast_cancer/spot_level_intensity_features.csv",row.names=1,header=T))
IF_texture= as.matrix(read.csv("/project/shared/xiao_wang/projects/MOCCA/data/Sprod_ready_data/visium/breast_cancer/spot_level_texture_features.csv",row.names=1,header=T))
rownames(E) ==rownames(C)
E= E/rowSums(E)*1e6
dim(E)
IF=cbind(IF_intens[rownames(E),],IF_texture[rownames(E),])
cbind(IF_intens,IF_texture)
?write.table
write.table(E,"/project/shared/xiao_wang/projects/MOCCA/data/Pseudotime/CPM_BC.txt",quote = F,sep = " ",row.names = T,col.names = T)
write.csv(IF,"/project/shared/xiao_wang/projects/MOCCA/data/Pseudotime/IF_mix0708.csv")
write.csv(cbind(IF_intens,IF_texture),"/project/shared/xiao_wang/projects/MOCCA/data/Pseudotime/spot_level_features.csv")
write.csv(IF,"/project/shared/xiao_wang/projects/MOCCA/data/Pseudotime/spot_level_features_intenf0f2.csv")
dim(E)
dim(C)
dim(IF)
counts_fn
...
C$seurat_clusters
head(IF)
IF= select(data.frame(IF_intens),starts_with("f0")|starts_with("f2")) ##F0_IF####
project_name = "IF_f0_f2"
um = T
...
Stack$cluster <- as.character(C[Stack$row,"seurat_clusters"])
head(Stack)
...
library(ggplot2)
if (nrow(Stack) >100000){
  Stack_plot =Stack[sample(nrow(Stack),100000),]
}else{
  Stack_plot =Stack
}

Stack_plot$cluster= factor(Stack_plot$cluster)

cat("The number of rows to plot is:",nrow(Stack_plot))
head(Stack_plot)
subset_stack = Stack[sample(nrow())]
nrow
ggplot()+
  geom_line(data=Stack_plot,aes(x=x1,y=y1,group=pair
                ,color=value
                ,alpha=value)
#            ,color="azure3"
#            ,alpha=0.3
            )+
  geom_point(data=data.frame(C),aes(x=X,y=Y,fill=as.character(seurat_clusters)),
             size = 1.5,shape=21,color="white",alpha=0.3)+
  xlab("X")+
  ylab("Y")+
#  geom_point(aes(color=cluster),size = 0.5)+
  ggtitle(paste0(project_name,"_Spatial connection of Detected Graph"))
#  geom_point(aes(color=celltype1)) +
#  scale_color_manual(values=c("palevioletred1", "seagreen4", "darkturquoise"))+
#  theme_bw()

pdf(paste0(output_path,"/",project_name,"_tsne_Connection.pdf"),width = 6,height = 6)
ggplot() + 
  geom_line(data=Stack_plot,aes(x=x1,y=y1,group=pair
                                ,color=value
                                ,alpha=value)
  ...
  geom_point()+
#  scale_color_manual(values=c("palevioletred1", "seagreen4", "darkturquoise"))+
  ggtitle(paste0(project_name,"_TSNE connection of Detected Graph"))
...
#             size = 1.5,shape=21,color="white",alpha=0.3)+
  ...
  ylab("UMAP2")+
  #  scale_color_manual(values=c("palevioletred1", "seagreen4", "darkturquoise"))+
  ggtitle(paste0(project_name,"_UMAP connection of Detected Graph"))
...
ggplot(Stack_plot,aes(x=x1,y=y1)) + 
  ...
  #  scale_color_manual(values=c("palevioletred1", "seagreen4", "darkturquoise"))+
  ggtitle(paste0(project_name,"_UMAP codnnection of Detected Graph"))

#plot(IF[,1],IF[,2])
plot(C$Y,C$X,pch=19,col=1+(IF[,1]<-2.5))
