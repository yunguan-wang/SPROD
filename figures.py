...
sns.lineplot(
    ...
    hue = 'Total_counts_bin', hue_order=['Q1','Q2','Q3','Q4'], lw=5)
...
oc_im_cts = np.log2(1+oc_im_cts.apply(lambda x: 1e3*x/x.sum(),axis=1).fillna(0))
oc_wt_cts = np.log2(1+oc_wt_cts.apply(lambda x: 1e3*x/x.sum(),axis=1).fillna(0))
ptprc_cor = oc_im_cts.corr()['PTPRC'].sort_values(ascending=False)[:51].index

#%%
sns.set_theme('paper',font_scale=2)
scatterplot_data = pd.concat(
    [oc_im_cts['PTPRC'],np.log2(1+img_features['f0_median'])], axis=1)
# scatterplot_data = scatterplot_data.append(
#     pd.concat([oc_wt_cts['PTPRC'],img_features['f0_median']], axis=1))
scatterplot_data.columns = ['PTPRC log2(CPK+1)', 'log2(CD45+1)']
scatterplot_data['Mean Top5% highest correlated with PTPRC'] = oc_im_cts[ptprc_cor].mean(axis=1)
# scatterplot_data['Dataset'] = ['Targeted']*len(oc_wt_cts) + ['Whole transcriptome']*len(oc_wt_cts)

for j in range(2):
    for col in scatterplot_data.columns[[0,2]]:
        cells = (scatterplot_data.index, high_qual_cells)[j]
        title = ['','high_quality'][j]
        g = sns.jointplot(
            data=scatterplot_data.loc[cells],
            x=col, y='log2(CD45+1)',
            bw_method = 0.2, fill=True, alpha=0.75, color='cyan', kind='kde', 
            xlim = (0,3)
            )
        cor = round(scatterplot_data.loc[cells,[col, 'log2(CD45+1)']].corr().iloc[0,1],2)
        _ = g.ax_marg_x.set_title('Pearson correlation: {}'.format(cor))
        plt.savefig(
            '/project/shared/xiao_wang/projects/MOCCA/figures/' + 
            'Visium_OC_{}_{}_scatterplot.pdf'.format(col, title),
...
for noise_level in range(1,6):
    ...
    x = np.arange(len(y))
    plt.plot(x, y, label = 'Noise level = 0.{}'.format(noise_level), lw=5)
...
# Visualize graph
graph = pd.read_csv(
    '/project/shared/xiao_wang/projects/MOCCA/data/grid_search_0.5n_clean_F/' + 
    '3.0_0.08_250.0_3.0_1.0_0.1_0.625_0.001/Detected_graph.txt',
    sep='\t')
spot_meta = pd.read_csv(
...
for i in r1000:
    ax.arrow(x0[i],y0[i],dx[i],dy[i], head_width=0, color='magenta', alpha=0.5,
    width=0.0005)
plt.legend(bbox_to_anchor=(1,0.5), loc = 'center left', markerscale=2)
