...
# Rscript /project/shared/xiao_wang/projects/MOCCA/code/Spatial_denoise/script/denoise_trial.R \
...
# 3 umap 0.1 250 10 0.5 1 0.01 -1 \
...
if (Sys.getenv("RSTUDIO") == "1") # for dev purpose
{
  ...
  args=c("Counts.txt","Spot_metadata.csv",
    "Spot_level_haralick_features.csv",3,"umap",0.1,250,10,0.5,1,0.01,-1,
    "/project/shared/xiao_wang/projects/MOCCA/code/Spatial_denoise",
    ".","")  
}
...
um = args[5] =="umap" | args[5] == "UMAP" # options for IF transformation. set to "umap" or "UMAP" if the IF needs to be tranformed by UMAP; otherwise the IF will be origial of PCA tranformed depend on whether N_PC is positive.
R_ratio=as.numeric(args[6]) # Spot neighborhood radius ratio, 0-1, radius=R*min(xmax-xmin,ymax-ymin)
U=as.numeric(args[7]) # perplexity, Tunable
K=as.numeric(args[8]) # latent space dimension, Tunable
...
d= as.numeric(args[12]) #option on W, set to postivie to prevent over-smooth, otherwise, set to -1 to use the original W.
script_path=args[13] # path to the software folder
output_path=args[14] # output path
project_name = args[15] # the name of this project or run
...
if (N_PC>0) {
  cat("PCA for IF is chosen!")
  IF=prcomp(IF)$x[,1:min(N_PC,dim(IF)[2])]
}else{
  if (um){
    cat("UMAP for IF is chosen!")
    library(umap)
    IF=umap(IF)$layout
  }else{
    cat("Non-transform on IF!")
  }
}
# Preprocessing the features with PCA or UMAP, optional
#######  spatial denoise  ################
cat("denoising...\n")
ptm <- proc.time()
...
write.table(round(E_denoised,d=5),
            paste(output_path,'/',project_name,'_Denoised_matrix.txt',sep=""),
            sep='\t',row.names = T,col.names = T,quote=F)

rownames(G)=colnames(G)=rownames(E)
write.table(round(G,d=5),
            paste(output_path,'/',project_name,'_Detected_graph.txt',sep=""),
            sep='\t',row.names = T,col.names = T,quote=F)

rownames(Y)=rownames(E)
colnames(Y)=paste("Component",1:dim(Y)[2],sep="")
write.table(round(Y,d=5),
            paste(output_path,'/',project_name,'_Latent_space.txt',sep=""),
            sep='\t',row.names = T,col.names = T,quote=F)
